...
| `max_chi2` | `25.0` | Reduced χ² above which a fit *may* be flagged `FLAG_BAD_CHI2` — see `max_rel_misfit`, which it is judged together with. Tighten (lower) for a cleaner peak map at the cost of more rejected fits; loosen if visibly good fits are being flagged bad. |
| `max_rel_misfit` | `0.05` | Rms fit residual as a fraction of the peak's **own height**, above which the fit is bad. A peak is flagged `FLAG_BAD_CHI2` only when it fails this *and* `max_chi2`. The reason for two tests: the fit weights every point by one scalar noise estimate taken from the background, so a reduced χ² measures the misfit in units of background noise and grows as (peak height / noise)² for any fixed fractional profile mismatch. On its own it rejects peaks for being well measured — on the archived DAC series the strongest reflection of the frame failed it in 1055 of 1288 frames while fitting to ~1% of its own height. Weak peaks are noise-limited and still governed by χ²; bright peaks are systematics-limited and judged on relative misfit. Raise it to keep more strong peaks, lower it to be stricter about profile shape. |
