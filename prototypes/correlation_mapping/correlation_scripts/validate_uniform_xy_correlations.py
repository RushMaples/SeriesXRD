#!/usr/bin/env python3
"""Validate a completed ``uniform-correlation-v2`` result directory.
...
PROFILE_ID = "uniform-correlation-v2"
DEFAULT_TOLERANCE = 1.0e-10
SHA256_RE = re.compile(r"^[0-9a-fA-F]{64}$")
MARKDOWN_LINK_RE = re.compile(r"\[[^\]]*\]\(([^)]+)\)")
FORMULA_ERROR_TOKENS = ("#REF!", "#DIV/0!", "#VALUE!", "#NAME?", "#NUM!", "#NULL!")
REQUIRED_SYNTHETIC_CASES = {
    "stationary_peak",
    "continuous_peak_shift",
    "known_area_ratio",
    "peak_appearance_disappearance",
    "adjacent_peaks",
    "crossing_peaks",
    "baseline_drift",
    "different_sampling_interval",
    "global_intensity_scaling",
    "missing_frame",
}
...
def validate_structure(run_dir: Path, state: ValidationState) -> None:
    ...
    workbook = run_dir / "UOTe_Handoff2_Correlation_Report_uniform_v2_20260714.xlsx"
    state.add("structure_workbook", workbook.is_file(), workbook.name)
    for directory in ("validation", "robustness", "comparison_to_legacy"):
...
def validate_manifests(run_dir: Path, state: ValidationState) -> None:
    ...
    binding_ok = True
    for family in ("peak_config", "window_config"):
    ...
    binding_code_sha = code_sha.get("uniform_profile_binding.py") if isinstance(code_sha, Mapping) else None
    state.add(
        ...
        {"uniform_profile_binding.py": binding_code_sha},
    )
...
def validate_official_acceptance_evidence(
    ...
    state.add(
        "official_both_channels_present",
        channels == ["spots", "fit"],
        {"channels": channels},
    )
    state.add(
    ...
    legacy_path = run_dir / "comparison_to_legacy" / "legacy_integrity.json"
    state.add("official_legacy_integrity_report_present", legacy_path.is_file(), str(legacy_path))
    ...
    if legacy_path.is_file():
        try:
            ...
            legacy = {}
    state.add(
        "official_legacy_directory_unchanged",
        parse_bool(legacy.get("unchanged")) is True
        and legacy.get("sha256_before") == legacy.get("sha256_after")
        and legacy.get("files") == legacy.get("files_after"),
        {
            "files_before": legacy.get("files"),
...
def validate_matrices(run_dir: Path, state: ValidationState, tolerance: float) -> list[MatrixItem]:
    ...
    for role, count in role_counts.items():
        if role in {"area", "location", "presence"}:
            all_channels_explicitly_zero = all(value[0] for value in zero_evidence.values())
            nonempty_count = sum(
...
def validate_run(run_dir: Path | str, tolerance: float = DEFAULT_TOLERANCE) -> dict[str, Any]:
    ...
    return {
        "validator": "validate_uniform_xy_correlations-v1",
        "profile": PROFILE_ID,
        "run_dir": str(root),
        "tolerance": tolerance,
        "checks": state.checks,
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("run_dir", type=Path, help="Completed uniform-v2 result directory")
