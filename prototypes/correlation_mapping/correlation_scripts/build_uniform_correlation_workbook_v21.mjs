...
/**
 * Build the uniform-correlation-v2.1 Excel report from public result files.
 *
...
function validationSummary(report, relativeSource) {
  ...
  // Some scientific validators deliberately expose a false-valued invariant
  // as a successful check (for example, ``cross_cut_interpolation_allowed``
  // must be false).  Once a report supplies its own top-level ``passed``
  // decision, do not reinterpret those domain-specific booleans as failures.
  const failedCheckCount = explicitPassed === true
    ? 0
    : explicitPassed === false
      ? Math.max(1, errors.length, checkValues.filter((value) => !value).length)
      : checkValues.filter((value) => !value).length;
  return {
...
function widthForColumn(header, rows) {
  ...
  if (/(track_id|segment_id|parent_track)/.test(key)) return 36;
...
function scientificEvidenceText(validationPassed, strictAuc, hasFitControl) {
  ...
  if (strictAuc >= 0.7) {
    return hasFitControl
      ? "严格 ACF 对近/远压力有较强区分，但仍需同时看 95% CI、direct control 和 fit control；它本身不能单独证明相变。"
      : "严格 ACF 对近/远压力有较强区分，但本数据没有 fit control；仍需看 95% CI 和 direct control，它本身不能单独证明相变。";
  }
  if (strictAuc >= 0.6) {
...
async function main() {
  ...
  let guiCrosscheck = null;
  const guiCrosscheckPath = path.join(
    resultRoot,
    "validation",
    "gui_crosscheck",
  ...
  try {
    guiCrosscheck = await readJsonRequired(guiCrosscheckPath);
  } catch {
    // A direct-manifest dataset may have no corresponding BulkXRD GUI HDF5.
  }
  const channels = Array.isArray(manifest.channels) && manifest.channels.length
    ? [...new Set(manifest.channels.map((item) => String(item).trim()).filter(Boolean))]
    : ["spots", "fit"];

  const peakRows = [];
  const acrossRows = [];
  const withinRows = [];
  const trackingAuditRows = [];
  for (const channel of channels) {
    ...
    for (const auditName of [
      ...
      const auditPath = path.join(resultRoot, channel, "per_peak", auditName);
      trackingAuditRows.push(
        ...addProvenance(await readCsvRequired(auditPath), relative(auditPath), {
          channel,
          audit_type: auditName.replace(/\.csv$/i, ""),
        }),
      );
    }
  }
  ...
  const guiCoverage = guiCrosscheck?.coverage ?? {};
  const guiSpotsScans = asNumber(guiCoverage.spots_gui_scans);
  const guiCorrelationScans = asNumber(guiCoverage.spots_correlation_scans);
  const guiFraction = asNumber(guiCoverage.spots_fraction);
  const guiGoodSummary = Array.isArray(guiCrosscheck?.main_v21_good_only_match_summary)
    ? guiCrosscheck.main_v21_good_only_match_summary[0] ?? null
    : null;
  ...
  const profile = firstDeepValue(manifest, ["profile", "profile_id", "algorithm_profile"], "uniform-correlation-v2.1");
  ...
  // Per Peak: fixed headline cell B5 is the spots official segment count.
  setupSheet(
    sheets["Per Peak"],
    "Per Peak / 自动径向峰结果",
    "v2.1 只连接双向一致且无歧义的边；切口两侧形成独立 segment。area/location 只在同 scan 双方可靠检出时计算，缺失保持 NaN。",
    10,
  );
  writeKpis(sheets["Per Peak"], 3, [
    { label: "Summary rows", value: peakRows.length, numberFormat: "#,##0", explanation: `当前 channels (${channels.join(", ")}) 的 official segment summary 总行数。` },
    { label: "Spots official radial segments", value: spotsTrackCount, numberFormat: "#,##0", explanation: "满足统一压力 support 的无歧义局部径向峰 segments；不能代表全部峰，也不能跨 CUT 解释为同一 hkl。" },
    { label: "Fit official radial segments", value: channels.includes("fit") ? fitTrackCount : null, numberFormat: "#,##0", explanation: channels.includes("fit") ? "fit control 独立自动追踪到的可靠 segments。" : "本数据没有 fit channel，因此这里留空，不写成 0。" },
  ...
  const perPeakTable = writeObjectTable(
    sheets["Per Peak"],
    12,
    peakRows,
    "PerPeakSummaryTable",
  ...
  writeObjectTable(
    sheets["Per Peak"],
    perPeakTable.endRow + 3,
    trackingAuditRows,
    "SegmentTrackingAuditTable",
    ["source_file", "channel", "audit_type", "segment_id", "parent_track_id", "decision", "cut_reason", "consensus_id", "pressure_GPa", "status", "note"],
  );
  // The audit table reuses column C for a shorter field and would otherwise
  // shrink the peak-summary track ID above it.  Keep deterministic segment
  // IDs fully visible in the beginner-facing preview.
  sheets["Per Peak"].getRange("C:C").format.columnWidth = 38;
  sheets["Per Peak"].getRange("D:E").format.columnWidth = 36;
  ...
  writeKpis(sheets["Across Frames"], 3, [
    { label: "Window-summary rows", value: acrossRows.length, numberFormat: "#,##0", explanation: `当前 channels (${channels.join(", ")})、三种 family 的 window summary 行数。` },
  ...
  const methodRows = [
    ...
    { analysis: "Radial tracking v2.1", uniform_method: "保持 v2 gate/cost/margin；每条候选边做双向 Hungarian、两端 margin 与 q-order crossing 审核，不合格就在当地 CUT。", beginner_meaning: "一个歧义点不再让整条轨迹作废；只保留安全连接，切口两侧分别成为局部 segment。" },
  ...
  setupSheet(
    sheets.Summary,
    "Uniform XY Correlation v2.1 / 结果总览",
  ...
  const summaryRows = [
    ...
    [
      "主数据自动找到多少个可靠 radial segments？",
      "='Per Peak'!B5",
      "这是 spots 的 official radial segments 数量，只代表可无歧义追踪的峰子集；1D XY 没有方位角，也不能跨 CUT 当成同一 hkl。",
      "#,##0",
    ],
    [
    ...
    [
      "GUI 交叉检查覆盖多少？",
      guiSpotsScans !== null && guiCorrelationScans !== null
        ? `="${guiSpotsScans}/${guiCorrelationScans} (${((guiFraction ?? 0) * 100).toFixed(1)}%)"`
        : '="不适用"',
      guiCrosscheck
        ? `现有 GUI H5 只覆盖 spots 的 scan048，因此 verdict=${guiCrosscheck.verdict}。Good-only 一对一匹配 ${guiGoodSummary?.matched ?? 0}/${guiGoodSummary?.gui_candidates ?? 0} 个 GUI peaks；fit/tungsten GUI H5 ${guiCoverage.fit_gui_h5_missing ? "缺失" : "已提供"}。这项 QC 不会反向调算法。`
        : "本数据没有可对应的 GUI H5；这里不写成通过，也不把它当成算法失败。",
      null,
    ],
  ];
  ...
  sheets.Summary.getRange("A4:A11").format.columnWidth = 34;
  sheets.Summary.getRange("B4:B11").format.columnWidth = 17;
  sheets.Summary.getRange("C4:C11").format.columnWidth = 74;
  ...
  sheets.Summary.getRange("A13").values = [[scientificEvidenceText(validationPassed, spotsStrictAuc, channels.includes("fit"))]];
  ...
  const output = {
    ...
    source_counts: {
      ...
      tracking_audit_rows: trackingAuditRows.length,
      ...
      gui_crosscheck_present: guiCrosscheck !== null,
