...
CLI: ``seriesxrd-spots`` (or ``python -m seriesxrd.analysis.spots``).
"""
    ...
    try:
        with h5py.File(str(src), "r") as h:
            ...
            if g is None or "tracks" not in g:
                return {"ok": False,
                        "error": "no /spots group — run seriesxrd-spots first"}
...
def export_spot_masks(
    ...
    *,
    ...
    Retention filters (composable):
      ...
      * ``match``          — reflection table for a target phase: keep obs
                             ...
                             when contaminant and target-phase lines overlap.
    ...
    with h5py.File(str(spath), "r") as h:
        if "spots" not in h or "obs" not in h["spots"]:
            raise ValueError(f"No /spots/obs in {spath} — run seriesxrd-spots "
                             "first.")
        og = h["spots/obs"]
...
def export_spot_tracks(
    ...
    *,
    ...
    with h5py.File(str(src), "r") as h:
        g = h.get("spots")
        if g is None or "tracks" not in g:
            raise ValueError(f"No /spots group in {src} — run seriesxrd-spots first.")
    ...
    readme = f"""Crystallite spot-track export (seriesxrd)
=============================================
    ...
    if args.match_only:
        ...
        with h5py.File(str(path), "r") as h:
            if "spots" not in h or "tracks" not in h["spots"]:
                print(f"[ERROR] no /spots/tracks in {path} — run seriesxrd-spots "
