"""Step 3b training pipeline — the learned scorer behind ``seriesxrd[ml]``.
...
Run on a workstation or compute node; the full guide is in
``docs/ml-training.md``::
    ...
    if reflections is None:
        if not pymatgen_available():
            raise RuntimeError("pymatgen is required to simulate reflections "
                               "(pip install seriesxrd[phases]), or pass reflections=.")
...
def _require_torch():
    try:
        ...
        raise RuntimeError(
            ...
            "extra with `pip install seriesxrd[ml]` (on a cluster, load the "
...
# ---------------------------------------------------------------------------
# CLI  (seriesxrd-ml-train)
# ---------------------------------------------------------------------------

def main(argv: "Optional[List[str]]" = None) -> int:
    p = argparse.ArgumentParser(
        prog="seriesxrd-ml-train",
        description="Train the Step-3b learned pair scorer (seriesxrd[ml]) on "
