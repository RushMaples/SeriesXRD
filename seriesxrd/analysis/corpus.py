"""Training-only CIF corpus tooling (``seriesxrd-corpus``).
...
job:
    ...
    seriesxrd-corpus fetch  ids.txt  ./training_cifs      # COD IDs, one per line
...
rejects into ``rejected/`` so ``seriesxrd-ml-train --cif-dir`` sees only usable,
...
def _default_parser(cif_path: Path) -> Dict[str, Any]:
    ...
    if not pymatgen_available():
        raise RuntimeError(
            "Screening needs pymatgen (pip install seriesxrd[phases]) — the same "
            "dependency seriesxrd-ml-train needs anyway.")
...
# ---------------------------------------------------------------------------
# CLI  (seriesxrd-corpus)
# ---------------------------------------------------------------------------

def main(argv: "Optional[List[str]]" = None) -> int:
    import argparse
    p = argparse.ArgumentParser(
        prog="seriesxrd-corpus",
        description="Build/screen a training-only CIF corpus for seriesxrd-ml-train.")
