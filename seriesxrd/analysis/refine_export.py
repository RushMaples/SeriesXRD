...
SeriesXRD's Step 3 identifies phases and estimates per-frame pressure, but it is
...
def _d_window_mask(radial: np.ndarray, unit: str,
    ...
    ``seriesxrd-spots --exclude-d`` (gasket W, diamond anvil lines, solid Ne).
...
def _xy_header(*, axis: str, native_unit: str, wavelength: "Optional[float]",
    ...
    return (
        "SeriesXRD refinement-export pattern\n"
        f"axis: {axis}\n"
...
def _write_readme(path: Path, *, unit: str, wavelength: "Optional[float]",
    ...
    text = f"""# SeriesXRD refinement hand-off bundle

This directory is a self-contained hand-off for Rietveld refinement (e.g. in
GSAS-II) of the phases SeriesXRD identified in this dataset. Nothing past this
point depends on SeriesXRD.
...
def main_gsas_raw(argv: "list[str] | None" = None) -> int:
    """CLI: ``seriesxrd-export-gsas reduced.h5 out_dir [options]``."""
    import argparse
    p = argparse.ArgumentParser(
        prog="seriesxrd-export-gsas",
...
def main(argv: "list[str] | None" = None) -> int:
    """CLI: ``seriesxrd-export-refinement analysis.h5 out_dir [options]``."""
    import argparse
    p = argparse.ArgumentParser(
        prog="seriesxrd-export-refinement",
