...
Examples
--------
    seriesxrd-analyze reduced.h5 --phases Au,Re --workers 0
    seriesxrd-analyze reduced.h5 --steps 12            # background + peaks only
    seriesxrd-analyze reduced.h5 --phases Au -o out.h5 --ml-export ds.npz
...
def _run(args) -> int:
    ...
    if "3" in steps:
        ...
        if not args.no_unknowns and mres.get("n_residual_peaks"):
            ...
            run_unknowns(
                analysis_path,
                link_tol_fwhm=args.unknown_link_tol_fwhm,
...
def main(argv: "list[str] | None" = None) -> int:
    ...
    p.add_argument("--unknown-max-gap", type=int, default=2,
                   help="Missing ordered samples tolerated in an unknown track. Default 2.")
    p.add_argument("--unknown-max-axis-gap", type=float, default=None,
                   help="Optional max physical-axis jump for pressure/temp/time tracking "
                        "(GPa, K, or seconds).")
    p.add_argument("--unknown-min-frames", type=int, default=3,
                   help="Minimum observations to keep an unknown track. Default 3.")
    p.add_argument("--unknown-jaccard", type=float, default=0.6,
                   help="Co-occurrence threshold for merging unknown tracks. Default 0.6.")
    p.add_argument("--no-unknown-axis-predictor", action="store_true",
                   help="Disable local slope prediction for pressure/temp/time tracking.")
    ...
    p.add_argument("--ml-scorer", default="",
                   help="Similarity scorer for --ml-rank: 'cosine' (default) or "
                        "'torch:<model.pt>' (a trained seriesxrd-ml-train export; "
                        "needs seriesxrd[ml]). Whatever it proposes, Step 3a verifies.")
    # ML export
