"""CLI entry point for the analysis GUI.

Runnable three ways:
    bulkxrd-analysis-gui --config <path>          (console script, pip install)
    python -m bulkxrd.analysis.run_gui --config <path>
    python bulkxrd/analysis/run_gui.py --config <path>
"""
...
if __package__ in (None, ""):
    ...
    from bulkxrd.analysis.gui import run_app
else:
...
_CONFIG_SEARCH_PATHS = [
    ...
    THIS_DIR / "analysis_session_config.json",
]
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Launch the Bulk-XRD Analysis GUI")
    ...
    if args.config:
        ...
        if cfg is None:
            print("[ERROR] Could not auto-find analysis_session_config.json.", flush=True)
            print("Searched:", flush=True)
            for p in _CONFIG_SEARCH_PATHS:
                print(f"  {p}", flush=True)
            print("", flush=True)
            print("Run with:  python -m bulkxrd.analysis.run_gui --config <path_to_config>", flush=True)
            return 1
