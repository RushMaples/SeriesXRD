...
Runnable three ways:
    bulkxrd-reduce-gui --config <path>            (console script, pip install)
    python -m bulkxrd.reduce.run_gui --config <path>
    python bulkxrd/reduce/run_gui.py --config <path>
"""
...
if __package__ in (None, ""):
    ...
    from bulkxrd.reduce.gui import run_app
else:
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Launch the Bulk-XRD Batch Reduction GUI")
    ...
    if args.config:
        ...
        if cfg is None:
            ...
            print("", flush=True)
            print("Run with:  python -m bulkxrd.reduce.run_gui --config <path_to_config>", flush=True)
            return 1
