"""Watch-folder (live / during-beamtime) reduction: ``bulkxrd-watch``.
...
class _LiveWriter:
    ...
    def __init__(self, h5_path: Path, npt_1d: int, settings: Dict[str, Any],
        ...
        with h5py.File(str(self.path), "w") as h5:
            h5.attrs.update({
                "tool": "bulkxrd.reduce", "tool_version": _proc.VERSION,
...
def main(argv: "list[str] | None" = None) -> int:
    ...
    if not cfg_path.is_file():
        print(f"[ERROR] reduction config not found: {cfg_path} — run the "
              f"Reduction GUI once (or bulkxrd --workspace ...) to create it.",
              flush=True)
        return 1
