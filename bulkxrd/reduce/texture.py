...
:mod:`bulkxrd.reduce.straighten`: a cake is ``(n_azimuthal, n_radial)``
...
def ring_profile(cake: np.ndarray, radial: np.ndarray, azimuthal_deg: np.ndarray,
    ...
    Mirrors :func:`bulkxrd.reduce.straighten.ring_centroids`'s baseline and
...
def texture_metrics(phi_deg: np.ndarray, intensity: np.ndarray) -> Dict[str, Any]:
    ...
    approach as :func:`bulkxrd.reduce.straighten.fit_waviness`, but on
...
def _pick_rings(cake: np.ndarray, radial: np.ndarray, dr: float, halfwidth: float,
    ...
    Same heuristic :func:`bulkxrd.reduce.straighten.straighten_cake` uses when
...
def main(argv: "list[str] | None" = None) -> int:
    """CLI: ``bulkxrd-texture reduced.h5 [--rings N] [--halfwidth W] [--dry-run]``."""
    import argparse
    p = argparse.ArgumentParser(
        prog="bulkxrd-texture",
