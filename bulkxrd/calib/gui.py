...
HELP = {
    ...
    "backend_dir":       "Folder containing the bulkxrd package code (auto-detected). Only change this to run the GUI/worker from a different code copy.",
    "python_exe":        "Python interpreter used to run the GUI and pyFAI processing. Prefer the bulkxrd conda env.",
...
class CalibrationApp:
    def __init__(self, config_path: "str | Path", parent=None):
        ...
        if parent is None:
            ...
            self.root = tk.Tk()
            self.root.title(f"{TOOL_NAME} Calibration Review")
    ...
    def _build_gui(self):
        ...
        ttk.Label(topbar, text=f"{TOOL_NAME} Calibration Review", font=("TkDefaultFont", 14, "bold")).pack(side="left")
        ttk.Button(topbar, text="Open Console Logs", command=self.open_console_logs).pack(side="right", padx=4)
        ...
        t1_master = self._scrollable(self.tabs["1 Session & Inputs"])
        ...
        paned_2 = ttk.PanedWindow(self.tabs["2 Masking & Inspection"], orient="vertical")
        ...
        # TAB 3: Generation
        self._tab_generate(self.tabs["3 pyFAI Generate"])

        # TAB 4: Viewer fills the full tab (log is now a separate top-bar window)
        self._tab_viewer(self.tabs["4 QA Dashboard"])

        # TAB 5: Final Expor
        self._tab_final(self.tabs["5 Final Export"])
    ...
    def show_about(self):
        ...
        for k in ("bulkxrd", "pyFAI", "numpy", "python", "platform"):
            if k in versions:
                lines.append(f"{k}: {versions[k]}")
        for k, v in versions.items():
            if k not in ("bulkxrd", "pyFAI", "numpy", "python", "platform"):
                lines.append(f"{k}: {v}")
        lines.append("")
    ...
    def open_env_settings(self):
        ...
        self.field(frame, "notebook_dir",        "Notebook folder",              "dir")
        self.field(frame, "backend_dir",         "Backend folder",       "dir")
    ...
    def _tab_generate(self, parent):
        ...
        self.ttk.Label(frame, text="Runs pyFAI in a worker subprocess (conda DLLs set in worker PATH on Windows).",
    ...
    def open_console_logs(self):
        ...
        self._log_window.title("Bulk-XRD Console Logs")
    ...
    def _recent_json_path() -> Path:
        return Path.home() / ".bulkxrd_recent.json"

    def _load_recent(self) -> dict:
        """Read ~/.bulkxrd_recent.json safely."""
        try:
    ...
    def _save_recent(self, data: dict):
        """Write ~/.bulkxrd_recent.json safely."""
        try:
            p = self._recent_json_path()
    ...
    def _open_session_config_dialog(self):
        ...
        try:
            ...
            if not path:
                return
            relaunch_cmd = f"python -m bulkxrd.calib.gui --config \"{path}\""
    ...
    def _qa_worker_done(self, returncode: int, out_json: str, gen_idx: int):
        ...
        try:
            ...
            self.nb.select(self.tabs["4 QA Dashboard"])
            self._refresh_final_save_list()
        except Exception as e:
    ...
    def save_accepted(self):
        ...
        try:
            ...
            handoff_json = str(Path(handoff["accepted_folder"]) / "metadata" / "handoff_for_next_notebook.json")
    ...
    def shutdown(self, confirm: bool = True) -> bool:
        ...
        """
        if confirm and self._qa_running:
            if not self.messagebox.askyesno(
                "QA generation running",
                "A QA generation is still running — close anyway?"
            ):
                return False
