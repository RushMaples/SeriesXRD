...
- **Fit-quality gate is two-sided and PER PEAK** (`max_chi2` on `chi2_local` AND `max_rel_misfit=0.05` on `rel_misfit`; a peak must fail both). Two regimes: a weak peak is noise-limited so its residual matters in units of the noise floor; a bright peak exposes systematic profile mismatch so what matters is the residual as a fraction of its own height. The fit weights every point by one scalar σ (the MAD *background* floor), so χ² alone was an SNR gate — on the archived UOTe series the strongest reflection of the frame failed it in 1055/1288 frames while fitting to ~1% of its own height, so the marker's main line was never attributed or subtracted downstream. Both measures are computed over the peak's own span from the full group model (neighbours accounted for, not counted as misfit) and against its own amplitude. `/peaks/chi2` still reports the *group's* reduced χ² for inspection but is never a rejection: a shared verdict is only ever more lenient (a_max ≥ a_j) — 121 peaks were excused by a bright neighbour while bad alone, none condemned by one. Threshold is calibrated, not derived: a healthy fit misfits by 1.3–3.0% median at every SNR band above 10, knee at ~5%. Effect: good peaks 66% → 98%, strongest-peak survival 16% → 98%, detection unchanged. `/peaks/chi2_local` + `/peaks/rel_misfit` are exposed so consumers can tier position-use vs intensity-use; nothing downstream tiers yet (that needs a false-attribution measurement on labelled data).
