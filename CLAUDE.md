...
- **Fit-quality gate is two-sided** (`max_chi2` AND `max_rel_misfit=0.05`; a peak must fail both). The fit weights every point by one scalar σ — the MAD noise floor of the *background* — so a reduced χ² measures the misfit in units of background noise and grows as (height/σ)² for a fixed fractional profile mismatch. χ² alone was therefore an SNR gate: on the archived UOTe series the strongest reflection of the frame failed it in 1055/1288 frames while fitting to ~1% of its own height, so the marker's main line was never attributed or subtracted downstream. The second clause is `σ·√χ² / amplitude` (rms residual as a fraction of peak height), shared across a jointly-fitted group via its tallest peak. Measured effect on that series: good peaks 80.4% → 98.1%, strongest-peak survival 17.8% → 98.3%, detection unchanged.
